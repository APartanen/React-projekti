...
*/
 ...
 //lomake - perustiedot 1
  const perusTiedot = (props) => {
    return <Lomake call={setTiedot1} array={tiedot1} kaikki = {kaikkitiedot} kaikkiCall = {setKaikkitiedot} />;
  };

 //lomake1 - yrityksen organisaatio
  const organisaatio = (props) => {
    return <Lomake1 call={setTiedot2} array={tiedot2}  kaikki = {kaikkitiedot} kaikkiCall = {setKaikkitiedot}/>;
  };

  
 //lomake2 - sivu 3 työsopimus
  const tyosopimus = (props) => {
    return <Lomake2 call={setTiedot3} array={tiedot3} kaikki = {kaikkitiedot} kaikkiCall = {setKaikkitiedot}/>;
  };

  const tyoymparisto = (props) => {
    return <Lomake9 call={setTiedot4} array={tiedot4} kaikki = {kaikkitiedot} kaikkiCall = {setKaikkitiedot}/>;
  };

  const tyotehtavat = (props) => {
    return <Lomake3 call={setTiedot5} array={tiedot5} kaikki = {kaikkitiedot} kaikkiCall = {setKaikkitiedot}/>;
  };

  const tehtavaanHaku = (props) => {
    return <Lomake4 call={setTiedot6} array={tiedot6} kaikki = {kaikkitiedot} kaikkiCall = {setKaikkitiedot} />;
  };

  const muuta = (props) => {
    return <Lomake5 call={setTiedot7} array={tiedot7} kaikki = {kaikkitiedot} kaikkiCall = {setKaikkitiedot}/>;
  };

  const viimeistely = (props) => {
    return <Lomake6 call={setTiedot8} array={tiedot8} kaikki = {kaikkitiedot} kaikkiCall = {setKaikkitiedot}/>;
  };
